...
function LoginPage() {
  ...
  return <main className="login-wrap"><div className="login-glow" /><div className="login-card"><div className="app-kicker">Cloud Version</div><h1>限時錦標賽記帳</h1><p className="login-text">用 Email Magic Link 登入。資料會儲存在雲端，不再只放在本機瀏覽器。</p><Input label="Email" type="email" value={email} onChange={setEmail} placeholder="you@example.com" /><button className="primary-button full" type="button" onClick={signIn} disabled={sending || !email}>{sending ? "寄送中..." : "寄送登入連結"}</button></div></main>;
}
...
function StatsPage({ records }) {
  ...
  const byVenue = useMemo(() => groupBy(filteredRecords, (r) => r.venue), [filteredRecords]);
...
function DataPage({ records, signOut }) {
  ...
  function exportCsv() {
    ...
    const rows = records.map((record) => [record.date,record.venue,record.displayEventName,record.buyIn,record.serviceFee,record.reentryCount,record.reentryBuyInTotal,record.reentryServiceFeeTotal,record.entries,record.totalCost,record.prize,record.netProfit,`${record.roi.toFixed(1)}%`,record.notes]);
